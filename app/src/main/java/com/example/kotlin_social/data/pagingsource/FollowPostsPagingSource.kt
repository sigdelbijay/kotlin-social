package com.example.kotlin_social.data.pagingsource
...
class FollowPostsPagingSource (
    ...
    override suspend fun load(params: LoadParams<QuerySnapshot>): LoadResult<QuerySnapshot, Post> {
        return try {
            ...
            var parsedPage = curPage!!.toObjects(Post::class.java)
                    .onEach { post ->
                        val user = db.collection("users").document(post.authorId)
            ...
            val nextPage = db.collection("posts")
                    .whereIn("authorId", if(chunks.isNotEmpty()) chunks[0] else listOf())
                    .orderBy("date", Query.Direction.DESCENDING)
